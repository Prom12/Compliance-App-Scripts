...
[Serializable]
public class ResultsCanvas
{
    public GameObject ResultsCanvasObject;
    public TMP_Text points;
    public TMP_Text redirectText;


    public ResultsCanvas(GameObject ResultsCanvasObject, TMP_Text points,TMP_Text redirectText){
        ResultsCanvasObject = ResultsCanvasObject;
        points = points;
        redirectText = redirectText;
    } 

    public void ChangePoints(int Points){
        points.SetText(Points + " Exercise Points (EP)");
    }
}

public class GeneralMainSceneUI : UIControl
{
    ...
    public void SetUI(){
        // Clear UI Content
        MainScreenCanvas.GetComponent<Canvas>().GetComponent<UIDynamicControl>().clearContent();

        if(GeneralScriptableObj.CheckGameState(GeneralScriptableObj.ExperimentState.Initial.ToString())){
            MainScreenCanvas.GetComponent<Canvas>().GetComponent<UIDynamicControl>().clearExerciseState();
            ...
            toggleUIMovie();
            MainScreenCanvas.GetComponent<Canvas>().GetComponent<UIDynamicControl>().UpdateUI();
            ...
            toggleUIGym();
            ExerciseScreenCanvas.GetComponent<Canvas>().GetComponent<UIDynamicControl>().UpdateUI();
            // MainScreenCanvasObject.SetActive(false);
    ...
    private void toggleUIMovie(){
        if (GeneralScriptableObj.CheckExperimentTypePermission()){
            ...
            MainScreenCanvas.GetComponent<Canvas>().GetComponent<UIDynamicControl>().sendSequentialMovie();
    ...
    private void toggleUIGym(){
        if (GeneralScriptableObj.CheckExperimentTypePermission()){
            ...
            ExerciseScreenCanvas.GetComponent<Canvas>().GetComponent<UIDynamicControl>().sendSequentialGym();
            // DisplayWhenQuitButtonIsClickedCanvas.GetComponent<Canvas>().GetComponent<UIDynamicControl>().sendSequentialGym();
            StartButton.gameObject.SetActive(true);
            MainScreenCanvasObject.SetActive(false);
            ExerciseScreenCanvasObject.SetActive(true);
